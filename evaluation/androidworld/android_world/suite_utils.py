...
def _print_and_save_realtime_summary(
    results: list[dict],
    ...
    Args:
        results: 所有任务的结果列表
        n_task_combinations: 每个任务的实例数（用于计算 pass@n）
        result_saver: 结果保存器（可选）
    ...
    # 合并任务元数据
    try:
        tagged_df = _merge_task_metadata(df)
    except Exception:
        tagged_df = df

    # 基本统计
    total_tasks = len(df)
    successful = df[df['is_successful'].apply(lambda x: x if pd.notna(x) else 0) > 0.5]
    num_successful = len(successful)
    num_failed = total_tasks - num_successful
    success_rate = num_successful / total_tasks if total_tasks > 0 else 0

    print(f"\n{'=' * 80}")
    print(f"{'OVERALL SUMMARY':^80}")
    print(f"{'=' * 80}")
    print(f"  Total tasks:  {total_tasks}")
    print(f"  Successful:   {num_successful} ({success_rate:.4f})")
    print(f"  Failed:       {num_failed} ({1 - success_rate:.4f})")
    ...
    pass_at_n = {}
    if n_task_combinations > 1 and _TASK_TEMPLATE_COLUMN in df.columns:
        print(f"\n{'=' * 80}")
        print(f"{'PASS @ N SUMMARY':^80}")
        print(f"{'=' * 80}")

        # 按任务模板分组，计算 pass@n
        grouped = df.groupby(_TASK_TEMPLATE_COLUMN)
        num_tasks = len(grouped)

        for n in range(1, n_task_combinations + 1):
            pass_count = 0
            for task_name, group in grouped:
                # 获取该任务的所有实例（按 instance_id 排序）
                if 'instance_id' in group.columns:
                    instances = group.sort_values('instance_id').to_dict('records')
                else:
                    instances = group.to_dict('records')

                # 检查前 n 个实例是否有成功的
                for i in range(min(n, len(instances))):
                    if instances[i].get('is_successful', 0) > 0.5:
                        pass_count += 1
                        break

            pass_rate = pass_count / num_tasks if num_tasks > 0 else 0
            pass_at_n[f"pass@{n}"] = {
                "pass_count": pass_count,
                "total_tasks": num_tasks,
                "pass_rate": round(pass_rate, 4),
            }
            print(f"  Pass@{n}:   {pass_count}/{num_tasks} ({pass_rate:.4f})")

        # 统计 1次内成功、2次内成功...n次内成功
        if 'instance_id' in df.columns:
            print(f"\n{'=' * 80}")
            print(f"{'SUCCESS COUNTS WITHIN FIRST N ATTEMPTS':^80}")
            print(f"{'=' * 80}")
            ...
            for n in range(1, n_task_combinations + 1):
                ...
                for task_name, group in grouped:
                    if 'instance_id' in group.columns:
                        ...
                        instances = group.to_dict('records')
                    successes_in_n = sum(1 for i in range(min(n, len(instances)))
                                         if instances[i].get('is_successful', 0) > 0.5)
                    within_n_success += successes_in_n

                # 计算总尝试次数（每个任务最多 n 次）
                total_attempts = num_tasks * n
                within_n_rate = within_n_success / total_attempts if total_attempts > 0 else 0
                pass_at_n[f"within_{n}_success"] = {
                    "total_successes": within_n_success,
                    "total_attempts": total_attempts,
                    "success_rate": round(within_n_rate, 4),
                }
                print(f"  Within {n} attempt(s): {within_n_success}/{total_attempts} ({within_n_rate:.4f})")

        print()

    by_difficulty = {}
    if 'difficulty' in tagged_df.columns or 'difficulty' in df.columns:
        diff_col = 'difficulty'
        print(f"\n{'=' * 80}")
        print(f"{'BY DIFFICULTY':^80}")
        print(f"{'=' * 80}")
        print(f"  {'Difficulty':<10} {'Completed':<10} {'Success':<8} {'Failed':<8} {'Rate':<12}")
        print(f"  {'-' * 50}")

        by_difficulty = {}
        for difficulty in ['easy', 'medium', 'hard']:
            if diff_col in tagged_df.columns:
                diff_df = tagged_df[tagged_df[diff_col] == difficulty]
            else:
                diff_df = df[df.get('difficulty', pd.Series()) == difficulty]
            if len(diff_df) == 0:
                continue

            diff_total = len(diff_df)
            diff_success = len(diff_df[diff_df['is_successful'].apply(lambda x: x if pd.notna(x) else 0) > 0.5])
            diff_failed = diff_total - diff_success
            diff_rate = diff_success / diff_total if diff_total > 0 else 0

            by_difficulty[difficulty] = {
                "total": diff_total,
                "successful": diff_success,
                "failed": diff_failed,
                "success_rate": round(diff_rate, 4),
            }
            print(f"  {difficulty:<10} {diff_total:<8} {diff_success:<8} {diff_failed:<8} {diff_rate:.4f}")

        print()

    by_tag = {}
    if 'tags' in tagged_df.columns:
        print(f"\n{'=' * 80}")
        print(f"{'BY TAG':^80}")
        print(f"{'=' * 80}")

        # 处理空标签
        tagged_df_copy = tagged_df.copy()
        tagged_df_copy['tags'] = tagged_df_copy['tags'].apply(_normalize_tags)

        exploded_df = tagged_df_copy.explode('tags')

        print(f"  {'Tag':<30} {'Done':<6} {'Success':<8} {'Failed':<8} {'Rate':<12}")
        print(f"  {'-' * 65}")
        ...
        for tag in exploded_df['tags'].unique():
            if pd.isna(tag):
                continue
            tag_df = exploded_df[exploded_df['tags'] == tag]
            tag_total = len(tag_df)
            tag_success = len(tag_df[tag_df['is_successful'].apply(lambda x: x if pd.notna(x) else 0) > 0.5])
            tag_failed = tag_total - tag_success
            tag_rate = tag_success / tag_total if tag_total > 0 else 0

            by_tag[str(tag)] = {
                "total": tag_total,
                "successful": tag_success,
                "failed": tag_failed,
                "success_rate": round(tag_rate, 4),
            }
            print(f"  {str(tag):<30} {tag_total:<6} {tag_success:<6} {tag_failed:<6} {tag_rate:.4f}")

        print()

    if _TASK_TEMPLATE_COLUMN in df.columns:
        print(f"\n{'=' * 80}")
        print(f"{'PER-TASK RESULTS':^80}")
        print(f"{'=' * 80}")
        print(f"  {'No.':<4} {'Task name':<45} {'Success':<8} {'Trials':<8} {'Rate':<10}")
        print(f"  {'-' * 75}")

        task_stats = df.groupby(_TASK_TEMPLATE_COLUMN).agg({
            'is_successful': ['count', 'mean'],
        }).reset_index()
        task_stats.columns = ['task_name', 'num_instances', 'success_rate']

        for idx, row in task_stats.iterrows():
            task_name = row['task_name']
            num_instances = int(row['num_instances'])
            success_rate = row['success_rate'] if pd.notna(row['success_rate']) else 0
            success_count = int(round(success_rate * num_instances)) if pd.notna(success_rate) else 0

            status_icon = "✅" if success_rate > 0.5 else "❌" if pd.notna(success_rate) else "⚠️"
            print(f"  {idx + 1:<4} {task_name:<45} {success_count:<6} {num_instances:<6} {success_rate:.4f} {status_icon}")

        print()

    # 保存到 JSON
    if result_saver:
        result_saver.update_summary(summary)
        if pass_at_n:
            result_saver.update_pass_at_n(pass_at_n)
        if by_difficulty:
            result_saver.update_by_difficulty(by_difficulty)
        if by_tag:
            result_saver.update_by_tag(by_tag)

    return summary


def run_with_realtime_summary(
    suite: Suite,
    agent: base_agent.EnvironmentInteractingAgent,
    checkpointer: checkpointer_lib.Checkpointer = checkpointer_lib.NullCheckpointer(),
    demo_mode: bool = False,
    ...
    result_saver=None,
    n_task_combinations: int = 1,
    check_episode_fn=None,
) -> list[dict[str, Any]]:
    """Runs eval suite with realtime summary printing and JSON saving.

    This function extends the standard run() with:
    1. Realtime progress and statistics printing
    2. Per-task results saved to JSON
    3. Pass@n statistics (when n_task_combinations > 1)
    4. Results by tag and difficulty

    Args:
        suite: The suite of tasks to run on.
        agent: An agent that interacts on the environment.
        checkpointer: Checkpointer for resuming from checkpoints.
        demo_mode: Whether to run in demo mode.
        return_full_episode_data: Whether to return full episode data.
        result_saver: ResultSaver instance for JSON output.
        n_task_combinations: Number of task instances per task (for pass@n).
        check_episode_fn: Optional function to check episode data.
    ...
    for name, instances in suite.items():
        ...
        for i, instance in enumerate(instances):
            ...
            # Transferring from old checkpoint.
            if instance_name in completed_tasks:
                completed_episodes = completed_tasks[instance_name]
                episodes_metadata.extend(completed_episodes)
            if instance_name in failed_tasks:
                episodes_metadata.extend(failed_tasks[instance_name])
            already_processed = (
            ...
            # Print task header
            print(
                f"\n--- {instance.name} ({i + 1}/{len(instances)}) [{current_task_num}/{total_tasks}] ---"
            )

            episode = _run_task(instance, run_episode, agent.env, demo_mode=demo_mode)
            ...
            # 获取任务元数据。部分 TaskEval 实例没有 difficulty/tags 属性，
            # 后续汇总会再从 task_metadata.json 按 task_template 补齐。
            episode['difficulty'] = getattr(instance, 'difficulty', None)
            episode['tags'] = getattr(instance, 'tags', None)

            episode_metadata = {k: episode[k] for k in metadata_fields}
            episode_metadata['difficulty'] = episode.get('difficulty')
            episode_metadata['tags'] = episode.get('tags')

            episodes_metadata.append(episode_metadata)

            if result_saver and hasattr(result_saver, 'update_task_result'):
                task_result = _merge_task_metadata(
                    pd.DataFrame([episode_metadata])
                ).to_dict('records')[0]
                result_saver.update_task_result(task_result)

            # 调用 suite_utils 中的实时统计函数
            _print_and_save_realtime_summary(
                results=episodes_metadata,
...
def _extract_task_metadata() -> pd.DataFrame:
  ...
  # task_name 是类名（如 AudioRecorderRecordAudio），用于和 episode 里的
  # task_template 匹配；原始 task_template 是自然语言 prompt，另存为 task_prompt。
  df = df.rename(columns={_TASK_TEMPLATE_COLUMN: _TASK_PROMPT_COLUMN})
  return df[
      ['task_name', _TASK_PROMPT_COLUMN, 'difficulty', 'optimal_steps', 'tags']
  ]


def _merge_task_metadata(result_df: pd.DataFrame) -> pd.DataFrame:
  """Merges task metadata into per-episode or aggregated result data."""
  if _TASK_TEMPLATE_COLUMN not in result_df.columns:
    return result_df

  metadata_df = _extract_task_metadata()
  merged_df = result_df.copy()
  if 'task_name' not in merged_df.columns:
    merged_df['task_name'] = merged_df[_TASK_TEMPLATE_COLUMN]

  merged_df = merged_df.merge(
      metadata_df, on=['task_name'], how='left', suffixes=('', '_metadata')
  )

  for column in ['difficulty', 'optimal_steps', 'tags', _TASK_PROMPT_COLUMN]:
    metadata_column = f'{column}_metadata'
    if metadata_column not in merged_df.columns:
      continue
    if column in merged_df.columns:
      merged_df[column] = merged_df[column].combine_first(
          merged_df[metadata_column]
      )
    else:
      merged_df[column] = merged_df[metadata_column]
    merged_df = merged_df.drop(columns=[metadata_column])

  return merged_df


def _normalize_tags(tags):
  """Normalizes missing task tags to a printable untagged bucket."""
  if isinstance(tags, np.ndarray):
    tags = tags.tolist()
  if isinstance(tags, list):
    normalized = [tag if tag else 'untagged' for tag in tags]
...
def process_episodes(
  ...
  tagged_result_df = _merge_task_metadata(result_df.reset_index())
