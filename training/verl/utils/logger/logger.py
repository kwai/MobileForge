...
class WandbLogger(Logger):
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        wandb.define_metric("total_epochs")
        wandb.define_metric("progress/step")
        wandb.define_metric("progress/total_steps", step_metric="progress/step")
        wandb.define_metric("progress/epoch", step_metric="progress/step")
        wandb.define_metric("progress/total_epochs", step_metric="progress/step")
        wandb.define_metric("progress/pct", step_metric="progress/step")
    ...
    def log(self, data: dict[str, Any], step: int) -> None:
        ...
        if total_steps > 0 and global_step >= 0:
            pct = min(global_step / total_steps * 100, 100)
            wandb.log(
                {
