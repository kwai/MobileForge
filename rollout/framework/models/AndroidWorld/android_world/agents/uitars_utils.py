...
def make_divisible(number, div):
    if div is not None:
        return int(-(-number // div) * div)
    return number


def transform_coordinates(coords, from_bounds, to_bounds, divisor=None):
    fx1, fy1, fx2, fy2 = from_bounds
    tx1, ty1, tx2, ty2 = to_bounds
    ratio_w = (tx2 - tx1) / (fx2 - fx1)
    ratio_h = (ty2 - ty1) / (fy2 - fy1)
    offset_x = tx1 - fx1 * ratio_w
    offset_y = ty1 - fy1 * ratio_h

    def _transform_coords(_c, div):
        if len(_c) == 2:
            ret = [_c[0] * ratio_w + offset_x, _c[1] * ratio_h + offset_y]
            return [make_divisible(x, div) for x in ret]
        elif len(_c) == 4:
            ret = [
                _c[0] * ratio_w + offset_x,
                _c[1] * ratio_h + offset_y,
                _c[2] * ratio_w + offset_x,
                _c[3] * ratio_h + offset_y,
            ]
            return [make_divisible(x, div) for x in ret]
        else:
            raise ValueError(f"Unsupported coords type {_c}")

    if isinstance(coords, (list, tuple)):
        if isinstance(coords[0], (list, tuple)):  # 多组坐标
            return [_transform_coords(c, divisor) for c in coords]
        else:  # 单组坐标
            return _transform_coords(coords, divisor)
    elif isinstance(coords, dict):
        return {k: _transform_coords(v, divisor) for k, v in coords.items()}
    else:
        raise NotImplementedError(f"Unsupported coords type {type(coords)}")
