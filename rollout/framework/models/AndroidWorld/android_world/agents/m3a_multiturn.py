...
# 新增一个用于后续多轮对话的增量PROMPT模板
INCREMENTAL_PROMPT_TEMPLATE = (
    "This is the summary of the last step: {summary}\n\n"
    "Based on this, the current screenshot, and the new UI element list below, please decide the next action.\n"
    "The numeric indexes are consistent with the ones in the labeled screenshot:\n{ui_elements}\n\n"
    "Your Answer:\n"
)
...
PROMPT_PREFIX = (
    ...
    '- Type text into the currently focused text field and then press the Enter key(this action only types'
    ' the text without clicking the field, so you need to click on'
    ' the target field first if needed):'
...
GUIDANCE = (
    ...
    ' in the list.\n'
    'Reason Content Requirements:\n'
    '- In your reasoning, describe UI elements and screen content using NATURAL LANGUAGE only\n'
    '- Use visual descriptions like "the blue button in the top right", "the search bar at the top", "the list item showing \'Messages\'"\n'
    '- Avoid mentioning technical details like index numbers, XML attributes, or bounding box coordinates in your reasoning\n'
    '- Focus on what a human user would see and describe on the screen\n'
    '- When referring to elements in your reasoning, use their visible text, icons, or position relative to other visible elements\n'
    '- Example good reasoning: "I can see the red \'Delete\' button at the bottom of the screen", "there is a settings icon (gear shape) in the top right corner", "the text field labeled \'Username\' is empty and ready for input"\n'
    '- Example bad reasoning: "I should click on element with index 5", "the clickable area at coordinates (100,200)", "the view with resource-id \'btn_submit\'"\n'
    '- Describe your reasoning process in terms of what you observe visually and why it helps achieve the goal\n'
    '- Connect your visual observations to the user\'s request in a logical, step-by-step manner\n'
    '- Note: While your reasoning should use natural language descriptions, you still need to use the correct index numbers in your JSON action output\n'
)
...
class M3A_MultiTurn(base_agent.EnvironmentInteractingAgent):
  ...
  def __init__(
    ...
    # 在初始化时设置好多轮对话的初始系统消息（使用OpenAI标准格式）
    initial_prompt = PROMPT_PREFIX + '\n' + GUIDANCE
    self.llm.messages = [{"role": "system", "content": initial_prompt}]
  ...
  def step(self, goal: str) -> base_agent.AgentInteractionResult:
    step_data = {
        ...
        'observation': None,  # Add observation to carry raw pixels
    }
    print('----------step ' + str(len(self.history) + 1))

    state = self.get_post_transition_state()
    step_data['observation'] = {'pixels': state.pixels.copy()}  # Store raw pixels in observation
    ...
    # 根据是否是第一轮对话，构建不同的Prompt
    if not self.history:  # 第一轮，发送完整PROMPT
        action_prompt = ACTION_SELECTION_PROMPT_TEMPLATE.format(
            goal=goal,
            history='You just started, no action has been performed yet.',
            ui_elements=before_ui_elements_list if before_ui_elements_list else 'Not available',
            additional_guidelines='', # 初始轮次在PROMPT_PREFIX中已经包含了GUIDANCE
        )
    else:  # 后续轮次，发送增量PROMPT
        last_summary = self.history[-1]['summary']
        action_prompt = INCREMENTAL_PROMPT_TEMPLATE.format(
    ...
    # 调用多轮对话API
    action_output, is_safe, raw_response = self.llm.predict_multiturn(
        action_prompt,
        [
...
Action: {{"action_type": "status", "goal_status": "infeasible"}}"""
    ...
    if (not reason) or (not action):
      ...
      # 设置action_output_json为None以避免KeyError
