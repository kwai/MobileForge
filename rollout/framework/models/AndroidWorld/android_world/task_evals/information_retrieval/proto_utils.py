...
def _cast_answers_to_type(
  ...
  if not match_types:
    return answers
  if match_types == [task_pb2.Expectation.MatchType.STRING_MATCH]:
    return [str(answer) for answer in answers]
  if match_types == [task_pb2.Expectation.MatchType.NUMBER_MATCH]:
    return [float(answer) for answer in answers]
  if match_types == [task_pb2.Expectation.MatchType.DATE_MATCH]:
    return [
  ...
  if match_types == [task_pb2.Expectation.MatchType.TIME_MATCH]:
    return [
