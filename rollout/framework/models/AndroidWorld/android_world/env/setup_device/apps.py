...
class SimpleSMSMessengerApp(AppSetup):
  ...
  def setup(cls, env: interface.AsyncEnv) -> None:
    ...
    sms_package = adb_utils.extract_package_name(
        adb_utils.get_adb_activity("simple sms messenger")
    )

    # Make Simple Messenger the default SMS app via ADB settings.
    # 通过 ADB 设置默认 SMS 应用（这在大多数情况下足够了）
    ...
    try:
      ...
      # 尝试点击 UI 元素来确认默认应用设置
      # 这些元素可能不存在（如果应用已经是默认应用或 UI 不同）
      # 使用 try-except 处理可能找不到元素的情况
      try:
        controller.click_element("SMS Messenger")
        time.sleep(2.0)
      except Exception:
        # 元素不存在，可能应用已经是默认应用或 UI 版本不同
        logging.debug("SMS Messenger button not found, app may already be default")
      ...
      try:
        ...
        # 元素不存在
        logging.debug("Set as default button not found, app may already be default")
    ...
    try:
      ...
      if check_result.generic and check_result.generic.output:
        current_default = check_result.generic.output
        if isinstance(current_default, bytes):
          current_default = current_default.decode('utf-8')
        current_default = current_default.strip()
        if current_default == sms_package:
          logging.info(f"Simple SMS Messenger successfully set as default SMS app")
        else:
          logging.warning(f"Default SMS app is {current_default}, expected {sms_package}")
    except Exception as e:
      logging.debug(f"Failed to verify default SMS app setting: {e}")
