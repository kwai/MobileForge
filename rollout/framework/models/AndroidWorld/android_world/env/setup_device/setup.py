...
def maybe_install_app(
  ...
  # 检查应用是否已安装，已安装则跳过下载
  try:
    adb_activity = adb_utils.get_adb_activity(app.app_name)
    if adb_activity:
      package_name = adb_utils.extract_package_name(adb_activity)
      if package_name:
        installed_packages = adb_utils.get_all_package_names(env.controller.env)
        if package_name in installed_packages:
          logging.info("App %s (%s) already installed, skipping download.", 
                       app.app_name, package_name)
          return
  except Exception as e:
    # 如果检查失败，继续尝试安装
    logging.warning("Failed to check if app %s is installed: %s", app.app_name, e)
