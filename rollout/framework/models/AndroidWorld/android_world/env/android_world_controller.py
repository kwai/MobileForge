...
# 确保使用本地的 android_env 模块（而非 pip 安装的版本）
# 本地版本的 loader.load() 接受 device_serial 参数
_FRAMEWORK_PATH = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__)))))
_ANDROID_ENV_PATH = os.path.join(_FRAMEWORK_PATH, "android_env")
if _ANDROID_ENV_PATH not in sys.path:
    sys.path.insert(0, _ANDROID_ENV_PATH)
...
def get_controller(
    ...
    Args:
        ...
        adb_path: ADB 路径
        grpc_port: gRPC 端口
        install_a11y_forwarding_app: 是否安装 accessibility_forwarder.apk。
            设置为 False 可跳过安装（假设 AVD 已预装）。
    """
